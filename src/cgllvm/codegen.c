...
static LLVMValueRef codegen_expr(const struct midpar_Expr *expr,
                                 const struct midllvm_Scope *scope,
                                 LLVMContextRef context, LLVMModuleRef mod,
                                 LLVMBuilderRef builder);
static LLVMValueRef codegen_expr_ref(const struct midpar_Expr *expr,
                                     const struct midllvm_Scope *scope,
                                     LLVMContextRef context, LLVMModuleRef mod,
                                     LLVMBuilderRef builder);
                                   ...
                                   LLVMContextRef context, LLVMModuleRef mod)
{
    ...
    int elem_size = lit->type == MIDLIT_STRINGTYPE_CHAR ? midtype_char_size * 8
                    : lit->type == MIDLIT_STRINGTYPE_WCHAR
                        ? midtype_wchar_size * 8
    ...
    for (mid_isize i = 0; i < len - 1; ++i) {
        ...
        case MIDLIT_STRINGTYPE_CHAR:
            elems[i] = LLVMConstInt(LLVMIntTypeInContext(context, elem_size),
                                    lit->c[i], midtype_char_signed);
            break;

        case MIDLIT_STRINGTYPE_WCHAR:
            elems[i] = LLVMConstInt(LLVMIntTypeInContext(context, elem_size),
                                    lit->wc[i], midtype_wchar_signed);
            break;
...
static LLVMValueRef codegen_ident_expr(const struct midpar_Expr *expr,
                                       const struct midllvm_Scope *scope,
...
static LLVMValueRef cast_value(LLVMValueRef val, const struct midpar_Type *src,
...
static void cast_arith_expr_operands(const struct midpar_Expr *expr,
...
static LLVMValueRef codegen_ptr_arith_expr(const struct midpar_Expr *expr,
                                           LLVMValueRef lhs, LLVMValueRef rhs,
                                           ...
                                           LLVMBuilderRef builder)
{
    ...
    midpar_Type_deinit(&deref);
    return ret;
}

static LLVMValueRef codegen_arith_expr(const struct midpar_Expr *expr,
                                       const struct midllvm_Scope *scope,
                                       LLVMContextRef context,
                                       LLVMModuleRef mod,
                                       LLVMBuilderRef builder)
...
static LLVMValueRef *get_call_args(const struct midpar_Expr *expr,
                                   const struct midllvm_Scope *scope,
                                   LLVMContextRef context, LLVMModuleRef mod,
                                   LLVMBuilderRef builder,
...
static LLVMValueRef codegen_call_expr(const struct midpar_Expr *expr,
                                      const struct midllvm_Scope *scope,
                                      LLVMContextRef context, LLVMModuleRef mod,
                                      LLVMBuilderRef builder)
{
...
static LLVMValueRef codegen_memb_sel(const struct midpar_Expr *expr,
                                     const struct midllvm_Scope *scope,
                                     bool load_ref, LLVMContextRef context,
                                     LLVMModuleRef mod, LLVMBuilderRef builder)
{
    const struct midpar_Expr *lhs_expr = &expr->info.args.arr[0];
    const struct midpar_Expr *rhs_expr = &expr->info.args.arr[1];

    LLVMValueRef lhs;
    if (expr->type == MIDPAR_EXPRTYPE_PTR_MEMB_SEL)
        lhs = codegen_expr(lhs_expr, scope, context, mod, builder);
    else
        lhs = codegen_expr_ref(lhs_expr, scope, context, mod, builder);

    assert(rhs_expr->type == MIDPAR_EXPRTYPE_IDENTIFIER);
...
static LLVMValueRef codegen_assign_expr(const struct midpar_Expr *expr,
                                        const struct midllvm_Scope *scope,
                                        bool load_ref, LLVMContextRef context,
                                        LLVMModuleRef mod,
                                        LLVMBuilderRef builder)
{
    ...
    if (load_ref || expr->type != MIDPAR_EXPRTYPE_ASSIGN) {
...
static void codegen_node(const struct midpar_ASTNode *node,
...
static void add_params_to_func(const struct midpar_FuncDecl *func,
                               struct midllvm_Scope *scope,
...
static struct midllvm_Scope *create_func_scope(
    ...
    const struct midpar_ASTNode *node, mid_isize func_ident,
...
static void codegen_func_body(const struct midpar_ASTNode *node,
                              ...
                              LLVMModuleRef mod)
{
    ...
    for (mid_isize i = 0; i < node->func_decl.nodes.len; ++i) {
        ...
        if (child->type == MIDPAR_ASTNODETYPE_RETURN) {
            ret_fnd = true;
            break;
        }
    }
...
static LLVMTypeRef *get_func_params(const struct midpar_ASTNode *node,
                                    LLVMContextRef context,
...
static void codegen_func_node(const struct midpar_ASTNode *node,
                              struct midllvm_Scope *scope,
                              struct midllvm_Allocators *allocs,
                              LLVMContextRef context, LLVMModuleRef mod)
{
...
static void call_ctor(const struct midpar_VarDeclInst *inst,
...
static void codegen_var_inst_node(const struct midpar_VarDeclInst *inst,
                                  struct midllvm_Scope *scope,
                                  LLVMContextRef context, LLVMModuleRef mod,
                                  LLVMBuilderRef builder)
{
...
static void codegen_var_node(const struct midpar_ASTNode *node,
                             struct midllvm_Scope *scope,
                             LLVMContextRef context, LLVMModuleRef mod,
                             LLVMBuilderRef builder)
{
...
static void codegen_ret_node(const struct midpar_ASTNode *node,
                             struct midllvm_Scope *scope,
                             LLVMContextRef context, LLVMModuleRef mod,
                             LLVMBuilderRef builder)
{
...
static void codegen_class_methods(const struct midpar_ASTNode *node,
                                  struct midllvm_Scope *scope,
                                  struct midllvm_Allocators *allocs,
                                  LLVMContextRef context, LLVMModuleRef mod)
{
...
static void codegen_class_node(const struct midpar_ASTNode *node,
                               struct midllvm_Scope *scope,
                               struct midllvm_Allocators *allocs,
                               LLVMContextRef context, LLVMModuleRef mod)
{
    /*
    char *name = midllvm_named_type_full_name(&(struct midpar_TypeNamed){
...
static void codegen_nmspace_node(const struct midpar_ASTNode *node,
                                 struct midllvm_Scope *scope,
                                 struct midllvm_Allocators *allocs,
                                 LLVMContextRef context, LLVMModuleRef mod)
{
