...
static LLVMValueRef codegen_expr(const struct MidParser_Expr *expr,
                                 const struct MidLLVM_Scope *scope,
                                 LLVMContextRef context, LLVMModuleRef mod,
                                 LLVMBuilderRef builder);
static LLVMValueRef codegen_expr_ref(const struct MidParser_Expr *expr,
                                     const struct MidLLVM_Scope *scope,
                                     LLVMContextRef context, LLVMModuleRef mod,
                                     LLVMBuilderRef builder);

static LLVMValueRef codegen_strlit(const struct MidLit_String *lit,
                                   LLVMContextRef context, LLVMModuleRef mod)
{
    mid_isize len = MidLit_strlit_len(lit) + 1;
    int elem_size = lit->type == MIDLIT_STRINGTYPE_CHAR ? MidTypes_char_size * 8
                    : lit->type == MIDLIT_STRINGTYPE_WCHAR
                        ? MidTypes_wchar_size * 8
                    : lit->type == MIDLIT_STRINGTYPE_CHAR16 ? 16
                                                            : 32;
...
static LLVMValueRef codegen_lit_expr(const struct MidParser_Expr *expr,
                                     LLVMContextRef context, LLVMModuleRef mod)
{
    if (MidParser_is_signed_integral_typespec(expr->ret.spec))
        return LLVMConstInt(
            MidLLVM_convert_parser_type(&expr->ret, context, false),
            expr->info.val.sint, true);
    else if (MidParser_is_unsigned_integral_typespec(expr->ret.spec))
        return LLVMConstInt(
            MidLLVM_convert_parser_type(&expr->ret, context, false),
            expr->info.val.uint, false);
    else if (MidParser_is_floating_typespec(expr->ret.spec))
        return LLVMConstReal(
            MidLLVM_convert_parser_type(&expr->ret, context, false),
            expr->info.val.flt);
    else if (MidParser_is_strlit(expr->type))
        return codegen_strlit(&expr->info.val.str, context, mod);
    else if (expr->type == MIDPARSER_EXPRTYPE_NULLPTR_LIT)
        return LLVMConstPointerNull(LLVMVoidType());
    else
        MID_CRASH("expr isn't a literal");
}

static LLVMValueRef codegen_ident_expr(const struct MidParser_Expr *expr,
                                       const struct MidLLVM_Scope *scope,
                                       bool load_ref, LLVMBuilderRef builder)
{
    auto ident = MidLLVM_find_ident_const(scope, expr->info.ident, NULL);
...
static LLVMValueRef get_implicit_this(const struct MidLLVM_Scope *scope)
{
    auto func = MidLLVM_find_func_scope_const(scope);
    auto f_ident = &func->parent->idents.arr[func->ident_idx];

    return LLVMGetParam(f_ident->val, 0);
}

static bool is_valid_array_subscr_ptr(const struct MidParser_Type *type)
{
    return type->spec == MIDPARSER_TYPESPEC_ARRAY ||
           MidParser_n_indir(type) > 0;
}

static LLVMValueRef codegen_subscr_expr(const struct MidParser_Expr *expr,
                                        const struct MidLLVM_Scope *scope,
                                        ...
                                        LLVMBuilderRef builder)
{
    ...
    auto type = MidLLVM_convert_parser_type(lhs_is_array ? lhs_t : rhs_t,
                                            context, false);
...
static LLVMValueRef cast_value(LLVMValueRef val,
                               const struct MidParser_Type *src,
                               const struct MidParser_Type *dest,
                               LLVMContextRef context, LLVMBuilderRef builder)
{
    if (MidParser_are_types_same(src, dest))
        return val;
    ...
    bool src_signed = MidParser_is_signed_integral_typespec(src->spec);
    bool src_int = MidParser_is_integral_typespec(src->spec);
    bool src_fp = MidParser_is_floating_typespec(src->spec);

    bool dest_int = MidParser_is_integral_typespec(dest->spec);
    bool dest_fp = MidParser_is_floating_typespec(dest->spec);
    ...
    if (src_indir == 0 && src_int && dest_indir > 0) {
        ...
        MID_CRASH("invalid ptr conversion");
    }
    ...
    MID_CRASH("couldn't cast assignment operand");
}
                                            ...
                                            LLVMBuilderRef builder)
{
    ...
    bool src_int = MidParser_is_integral_typespec(src->spec);
    bool src_fp = MidParser_is_floating_typespec(src->spec);
    i32 src_rank = MidParser_typespec_conv_rank(src->spec);

    bool dest_int = MidParser_is_integral_typespec(dest->spec);
    bool dest_fp = MidParser_is_floating_typespec(dest->spec);
    i32 dest_rank = MidParser_typespec_conv_rank(dest->spec);
...
static void cast_arith_expr_operands(const struct MidParser_Expr *expr,
                                     LLVMValueRef *lhs, LLVMValueRef *rhs,
                                     LLVMContextRef context,
                                     LLVMBuilderRef builder)
{
    if (!MidParser_is_binop(expr->type))
        return;
...
static LLVMValueRef codegen_ptr_arith_expr(const struct MidParser_Expr *expr,
                                           LLVMValueRef lhs, LLVMValueRef rhs,
                                           bool add, LLVMContextRef context,
                                           LLVMBuilderRef builder)
{
    assert(MidParser_n_indir(&expr->ret) > 0);

    bool lhs_ptr = MidParser_n_indir(&expr->info.args.arr[0].ret) > 0;
    ...
    auto deref = MidParser_deref_type(&expr->ret, NULL);
    auto ret = LLVMBuildGEP2(
        builder, MidLLVM_convert_parser_type(&deref, context, false), ptr, &off,
        1, "");

    MidParser_Type_deinit(&deref);
    return ret;
}

static LLVMValueRef codegen_arith_expr(const struct MidParser_Expr *expr,
                                       const struct MidLLVM_Scope *scope,
                                       LLVMContextRef context,
                                       LLVMModuleRef mod,
                                       LLVMBuilderRef builder)
{
    ...
    case MIDPARSER_EXPRTYPE_DIV:
        if (MidParser_is_floating_typespec(expr->ret.spec))
            return LLVMBuildFDiv(builder, lhs, rhs, "");
        else if (MidParser_is_signed_integral_typespec(expr->ret.spec))
    ...
    case MIDPARSER_EXPRTYPE_ADD:
        if (MidParser_n_indir(&expr->ret) == 0)
    ...
    default:
        MID_CRASH("expr isn't an arithmetic operator");
    }
}
                                   ...
                                   mid_isize *out_n_args)
{
    ...
    if (MidParser_func_is_ctor(&expr->node->func_decl)) {
        auto type = MidParser_implicit_this_type(&expr->node->func_decl);
        ...
        assert(MidParser_is_memb_sel(lhs->type));
...
static LLVMValueRef codegen_call_expr(const struct MidParser_Expr *expr,
                                      const struct MidLLVM_Scope *scope,
                                      LLVMContextRef context, LLVMModuleRef mod,
                                      LLVMBuilderRef builder)
{
    char *name = MidLLVM_mangle_func(&expr->node->func_decl);

    auto root = MidLLVM_find_root_scope_const(scope);
    auto func = MidLLVM_find_ident_const(root, name, NULL);
...
static LLVMValueRef codegen_memb_sel(const struct MidParser_Expr *expr,
                                     const struct MidLLVM_Scope *scope,
                                     bool load_ref, LLVMContextRef context,
                                     LLVMModuleRef mod, LLVMBuilderRef builder)
{
    const struct MidParser_Expr *lhs_expr = &expr->info.args.arr[0];
    const struct MidParser_Expr *rhs_expr = &expr->info.args.arr[1];

    LLVMValueRef lhs;
    if (expr->type == MIDPARSER_EXPRTYPE_PTR_MEMB_SEL)
        lhs = codegen_expr(lhs_expr, scope, context, mod, builder);
    else
        lhs = codegen_expr_ref(lhs_expr, scope, context, mod, builder);

    assert(rhs_expr->type == MIDPARSER_EXPRTYPE_IDENTIFIER);

    auto class_ = &MidSema_deref_identptr(&lhs_expr->ret.named)->decl->class_;
    mid_isize idx =
        MidLLVM_class_field_to_struct_field_idx(class_, rhs_expr->info.ident);
    if (idx == -1)
        return NULL;

    auto struct_type = MidLLVM_create_struct(
        &MidSema_deref_identptr(&lhs_expr->ret.named)->decl->class_, context);
    ...
    auto res_t = MidLLVM_convert_parser_type(&expr->ret, context, true);
    return LLVMBuildLoad2(builder, res_t, ptr, "");
}

static LLVMValueRef codegen_assign_expr(const struct MidParser_Expr *expr,
                                        const struct MidLLVM_Scope *scope,
                                        bool load_ref, LLVMContextRef context,
                                        LLVMModuleRef mod,
                                        LLVMBuilderRef builder)
{
    ...
    if (load_ref || expr->type != MIDPARSER_EXPRTYPE_ASSIGN) {
        deref_lhs = LLVMBuildLoad2(
            builder,
            MidLLVM_convert_parser_type(&lhs_expr->ret, context, false), lhs,
            "");
    }
    ...
    default:
        MID_CRASH("not an assignment expr");
    }
                                 ...
                                 LLVMBuilderRef builder)
{
    ...
    if (MidParser_is_numlit(expr->type))
        ...
        printf("expr at %" PRIi32 ":%" PRIi32 "\n", expr->tok->pos.line,
               expr->tok->pos.column);
        MID_CRASH("bad expr type\n");
    }
}

static void codegen_node(const struct MidParser_ASTNode *node,
                         struct MidLLVM_Scope *scope,
                         struct MidLLVM_Allocators *allocs,
                         LLVMContextRef context, LLVMModuleRef mod,
                         LLVMBuilderRef builder);

static struct MidLLVM_Scope *create_scope(struct MidLLVM_Scope *scope,
                                          struct MidLLVM_Allocators *allocs,
                                          const struct MidParser_ASTNode *node)
{
    struct MidLLVM_Scope *ret;
    MidGen_bumpmalloc(&allocs->scope, &ret);
    *ret =
        (struct MidLLVM_Scope){.parent = scope, .node = node, .ident_idx = -1};

    return ret;
}

static void add_params_to_func(const struct MidParser_FuncDecl *func,
                               struct MidLLVM_Scope *scope,
                               ...
                               LLVMBuilderRef builder)
{
    for (mid_isize i = 0; i < func->params.len; ++i) {
        ...
        struct MidLLVM_Ident ident = {.name = strdup(param->name)};
...
static struct MidLLVM_Scope *create_func_scope(
    struct MidLLVM_Scope *scope, struct MidLLVM_Allocators *allocs,
    const struct MidParser_ASTNode *node, mid_isize func_ident,
    LLVMContextRef context, LLVMValueRef func_val, LLVMBuilderRef builder)
{
    struct MidLLVM_Scope *ret = create_scope(scope, allocs, node);
...
static void codegen_func_body(const struct MidParser_ASTNode *node,
                              mid_isize func_ident,
                              struct MidLLVM_Scope *parent_scope,
                              struct MidLLVM_Allocators *allocs,
                              ...
                              LLVMModuleRef mod)
{
    ...
    for (mid_isize i = 0; i < node->func_decl.nodes.len; ++i) {
        ...
        if (child->type == MIDPARSER_ASTNODETYPE_RETURN) {
            ret_fnd = true;
            break;
        }
    }
...
static LLVMTypeRef *get_func_params(const struct MidParser_ASTNode *node,
                                    LLVMContextRef context,
                                    mid_isize *out_n_params)
{
    bool implicit_this =
        MidParser_func_takes_implicit_this(&node->func_decl, true);
    ...
    for (mid_isize i = 0; i < node->func_decl.params.len; ++i) {
        params[i + implicit_this] = MidLLVM_convert_parser_type(
...
static void codegen_func_node(const struct MidParser_ASTNode *node,
                              struct MidLLVM_Scope *scope,
                              struct MidLLVM_Allocators *allocs,
                              LLVMContextRef context, LLVMModuleRef mod)
{
    ...
    struct MidLLVM_Ident ident = {};

    auto ret_type =
        node->func_decl.is_tor
            ? LLVMVoidTypeInContext(context)
            : MidLLVM_convert_parser_type(&node->func_decl.ret, context, true);
    ident.type =
        LLVMFunctionType(ret_type, params, n_params, node->func_decl.variadic);
    free(params);

    ident.name = MidLLVM_mangle_func(&node->func_decl);
...
static void call_ctor(const struct MidParser_VarDeclInst *inst,
                      const struct MidLLVM_Ident *ident,
                      struct MidLLVM_Scope *scope, LLVMContextRef context,
                      LLVMModuleRef mod, LLVMBuilderRef builder)
{
    mid_isize n_args = inst->ctor.args.len + 1;
    LLVMValueRef *args = Mid_malloc(n_args * sizeof(*args));

    args[0] = ident->val;

    for (mid_isize i = 0; i < inst->ctor.args.len; ++i) {
        auto arg = &args[i + 1];
    ...
    char *name = MidLLVM_mangle_func(inst->ctor.ctor);
    auto root = MidLLVM_find_root_scope_const(scope);
    auto func = MidLLVM_find_ident_const(root, name, NULL);
...
static void codegen_var_inst_node(const struct MidParser_VarDeclInst *inst,
                                  struct MidLLVM_Scope *scope,
                                  LLVMContextRef context, LLVMModuleRef mod,
                                  LLVMBuilderRef builder)
{
    ...
    struct MidLLVM_Ident ident = {.name = strdup(inst->name)};

    ident.type = MidLLVM_convert_parser_type(&inst->type, context, false);
    if (inst->type.lv_ref)
        ident.val =
            codegen_expr_ref(inst->init.expr, scope, context, mod, builder);
    else
    ...
    } else if (inst->init.expr && !inst->type.lv_ref) {
...
static void codegen_var_node(const struct MidParser_ASTNode *node,
                             struct MidLLVM_Scope *scope,
                             LLVMContextRef context, LLVMModuleRef mod,
                             LLVMBuilderRef builder)
{
...
static void codegen_ret_node(const struct MidParser_ASTNode *node,
                             struct MidLLVM_Scope *scope,
                             LLVMContextRef context, LLVMModuleRef mod,
                             LLVMBuilderRef builder)
{
...
static void codegen_class_methods(const struct MidParser_ASTNode *node,
                                  struct MidLLVM_Scope *scope,
                                  struct MidLLVM_Allocators *allocs,
                                  LLVMContextRef context, LLVMModuleRef mod)
{
    for (mid_isize i = 0; i < node->class_.childs.len; ++i) {
...
static void codegen_class_node(const struct MidParser_ASTNode *node,
                               struct MidLLVM_Scope *scope,
                               struct MidLLVM_Allocators *allocs,
                               LLVMContextRef context, LLVMModuleRef mod)
{
    /*
    char *name = MidLLVM_named_type_full_name(&(struct MidParser_TypeNamed){
        .parent = node->class_.parent, .ident = node->class_.ident_idx});

    auto type = LLVMStructCreateNamed(context, name);
    auto fields = MidLLVM_class_to_struct_fields(&node->class_, context);
...
static void codegen_nmspace_node(const struct MidParser_ASTNode *node,
                                 struct MidLLVM_Scope *scope,
                                 struct MidLLVM_Allocators *allocs,
                                 LLVMContextRef context, LLVMModuleRef mod)
{
                         ...
                         LLVMBuilderRef builder)
{
    ...
    case MIDPARSER_ASTNODETYPE_ROOT:
        MID_CRASH("root node encountered within AST");
...
void MidLLVM_codegen(const struct MidParser_ASTNode *root)
{
    ...
    LLVMModuleRef mod =
        LLVMModuleCreateWithNameInContext(MidCMD_get_args()->src, context);

    struct MidLLVM_Allocators allocs = {};
    struct MidLLVM_Scope root_scope = {};

    for (mid_isize i = 0; i < root->root.len; ++i)
        codegen_node(root->root.arr[i], &root_scope, &allocs, context, mod,
                     NULL);

    MidLLVM_Scope_deinit(&root_scope);
    MidLLVM_Allocators_deinit(&allocs);
