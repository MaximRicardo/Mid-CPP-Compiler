...
void middiag_print(const struct mid_Diag *diag)
{
    ...
    case MIDDIAG_TYPE_ERROR:
        printf("%serror%s: ", midcmd_ansi_red, midcmd_ansi_reset);
        break;

    case MIDDIAG_TYPE_WARNING:
        printf("%swarning%s: ", midcmd_ansi_magenta, midcmd_ansi_reset);
        break;

    case MIDDIAG_TYPE_NOTE:
        printf("%snote%s: ", midcmd_ansi_cyan, midcmd_ansi_reset);
        break;
    }
    printf("%s\n", diag->msg);
    ...
    for (mid_isize i = 0; i < n_digits; ++i)
        putchar(' ');
    printf(" | ");
    midcmd_prt_column_arrow(diag->pos.column);
}
                                             ...
                                             enum middiag_ErrT type)
{
    return (struct mid_Diag){
        .pos = tok->pos,
        .line = tok->line,
        .msg = midcmd_fmt_to_str("unexpected %s", name),
        .err = type,
        .type = MIDDIAG_TYPE_ERROR,
    };
}
                                            ...
                                            enum middiag_ErrT type)
{
    return (struct mid_Diag){
        .pos = tok->pos,
        .line = tok->line,
        .msg = midcmd_fmt_to_str("'%s' redefined", name),
        .err = type,
        .type = MIDDIAG_TYPE_ERROR,
    };
}
                                             ...
                                             enum middiag_ErrT type)
{
    return (struct mid_Diag){
        .pos = tok->pos,
        .line = tok->line,
        .msg = midcmd_fmt_to_str("undeclared identifier '%s'", name),
        .err = type,
        .type = MIDDIAG_TYPE_ERROR,
    };
}
                                            ...
                                            enum middiag_ErrT type)
{
    return (struct mid_Diag){
        .pos = tok->pos,
        .line = tok->line,
        .msg = midcmd_fmt_to_str("undeclared function '%s'", name),
        .err = type,
        .type = MIDDIAG_TYPE_ERROR,
    };
}
                                           ...
                                           enum middiag_ErrT type)
{
    return (struct mid_Diag){
        .pos = tok->pos,
        .line = tok->line,
        .msg = midcmd_fmt_to_str("type-id can't be named"),
        .err = type,
        .type = MIDDIAG_TYPE_ERROR,
    };
}
