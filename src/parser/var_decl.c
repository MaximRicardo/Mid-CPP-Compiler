    ...
    struct Parser_Allocators *allocs, struct DiagVec *diags)
{
    *inst = (struct Parser_VarDeclInst){};
    ...
    if (has_init) {
        ...
        return Parser_parse_var_decl_inst_def(
            toks, expr_start, end_types, n_end_types, inst, res, allocs, diags);
                              ...
                              struct DiagVec *diags)
{
    *decl = (struct Parser_VarDecl){};
    ...
    isize_t i = base_end;
    do {
        gen_dynpush(&decl->insts, (struct Parser_VarDeclInst){});
        auto inst = &decl->insts.arr[decl->insts.len - 1];
